...
class ModsThing {
public:
    static void ExcelFly() {
        auto *rb = (Rigidbody *) GameObject::Find("GorillaPlayer")->GetComponent(
                Rigidbody::GetType());
        if (XRInput::GetBoolFeature(PrimaryButton, Left)) {
            rb->GetVelocity() +=
                    -GameObject::Find("LeftHand Controller")->GetTransform()->GetRight() / 2.f;
        }
        if (XRInput::GetBoolFeature(PrimaryButton, Right)) {
            rb->GetVelocity() +=
                    GameObject::Find("RightHand Controller")->GetTransform()->GetRight() / 2.f;
        }
    }
    static void TFly() {
        auto *rb = (Rigidbody *) GameObject::Find("GorillaPlayer")->GetComponent(Rigidbody::GetType());
        if (XRInput::GetBoolFeature(SecondaryButton, Left)) {
            rb->SetVelocity(Vector3(0.f, 0.01f, 0.f));
        }
        if (XRInput::GetBoolFeature(TriggerButton, Left)) {
            GameObject::Find("GorillaPlayer")->GetTransform()->SetPosition(GameObject::Find("GorillaPlayer")->GetTransform()->GetPosition() + GameObject::Find("LeftHand Controller")->GetTransform()->GetForward() * 0.45f);
            rb->GetVelocity() += GameObject::Find("RightHand Controller")->GetTransform()->GetRight() / 2.f;
        }
    }
    ...
    static void Platforms() {
        if (XRInput::GetBoolFeature(GripButton, Left)) {
            if (!PlatLonce) {
                PlatL = GameObject::CreatePrimitive(Cube);
                auto *rend = (Renderer *) PlatL->GetComponent(Renderer::GetType());
                rend->GetMaterial()->SetColor(Color::magenta);
                PlatL->GetTransform()->SetLocalScale(Vector3(0.2f, 0.2f, 0.2f));
                PlatL->GetTransform()->SetPosition(
                        GameObject::Find("LeftHand Controller")->GetTransform()->GetPosition());
                PlatLonce = true;
            }
        } else {
            if (PlatLonce) {
        ...
        if (XRInput::GetBoolFeature(GripButton, Right)) {
            if (!PlatRonce) {
                PlatR = GameObject::CreatePrimitive(Cube);
                auto *rend = (Renderer *) PlatR->GetComponent(Renderer::GetType());
                rend->GetMaterial()->SetColor(Color::magenta);
                PlatR->GetTransform()->SetLocalScale(Vector3(0.2f, 0.2f, 0.2f));
                PlatR->GetTransform()->SetPosition(
                        GameObject::Find("RightHand Controller")->GetTransform()->GetPosition());
                PlatRonce = true;
            }
        } else {
            if (PlatRonce) {
    ...
    static void UpsideDownMonke() {
        GameObject::Find("GorillaPlayer")->GetTransform()->SetRotation(
                Quaternion::FromEuler(0.f, 0.f, 180.f));
    }

    static void NoUpsideDownMonke() {
        GameObject::Find("GorillaPlayer")->GetTransform()->SetRotation(
                Quaternion::FromEuler(0.f, 0.f, 0.f));
    }

    /*static void GhostMonke() {
        if (PhotonNetwork::GetInRoom()) {
            auto rig = GlobalNamespace::GorillaTagger::get_Instance()->myVRRig<GlobalNamespace::VRRig *>();
            auto *d = (Behaviour *) rig;
            if (XRInput::GetBoolFeature(GripButton, Left)) {
                d->SetEnabled(false);
            } else {
                d->SetEnabled(true);
    ...
    static void InvisMonke() {
        if (PhotonNetwork::GetInRoom()) {
            auto rig = GlobalNamespace::GorillaTagger::get_Instance()->myVRRig<GlobalNamespace::VRRig *>();
            auto *d = (Behaviour *) rig;
            if (XRInput::GetBoolFeature(GripButton, Left)) {
                d->SetEnabled(false);
                d->GetTransform()->SetPosition(Vector3(9999.f, 9999.f, 9999.f));
            } else {
                d->SetEnabled(true);
            }
        }
    }*/
    static void ColossalSettings() {
        auto rig = (Rigidbody *) GameObject::Find("GorillaPlayer")->GetComponent(
                Rigidbody::GetType());
        if (XRInput::GetBoolFeature(GripButton, Right)) {
            if (DoOnce2) {
                maxD2 = 1.f;
            ...
            Physics::Raycast(
                    GameObject::Find("RightHand Controller")->GetTransform()->GetPosition(),
                    -GameObject::Find("RightHand Controller")->GetTransform()->GetRight(),
                    raycastHit, 1.f, layers);
            RaycastHit raycastHit2;
            Physics::Raycast(GameObject::Find("LeftHand Controller")->GetTransform()->GetPosition(),
                             GameObject::Find("LeftHand Controller")->GetTransform()->GetRight(),
                             raycastHit2, 1.f, layers);
            ...
            if (dist2 < maxD2) {
                vel2 = normal2 * (7.8f * Time::GetDeltaTime());
                rig->SetVelocity(rig->GetVelocity() - vel2);
            } else {
                rig->SetUseGravity(true);
            }
        } else {
            rig->SetUseGravity(true);
        }
    ...
    static void Ghost() {
        ...
        if (XRInput::GetBoolFeature(GripButton, Right)) {
            ...
            Physics::Raycast(
                    GameObject::Find("RightHand Controller")->GetTransform()->GetPosition(),
                    -GameObject::Find("RightHand Controller")->GetTransform()->GetRight(),
                    raycastHit, 2.f, layers);
            RaycastHit raycastHit2;
            Physics::Raycast(GameObject::Find("LeftHand Controller")->GetTransform()->GetPosition(),
                             GameObject::Find("LeftHand Controller")->GetTransform()->GetRight(),
                             raycastHit2, 2.f, layers);
            if (raycastHit2.distance > raycastHit.distance) {
                normal2 = raycastHit.normal;
                dist2 = raycastHit.distance;
            } else {
            ...
            if (dist2 < maxD2) {
                vel2 = normal2 * (8.8f * Time::GetDeltaTime());
                rig->SetVelocity(rig->GetVelocity() - vel2);
            } else {
                rig->SetUseGravity(true);
            }
    ...
    static void Blatant() {
        ...
        if (XRInput::GetBoolFeature(GripButton, Right)) {
            ...
            Physics::Raycast(
                    GameObject::Find("RightHand Controller")->GetTransform()->GetPosition(),
                    -GameObject::Find("RightHand Controller")->GetTransform()->GetRight(),
                    raycastHit, 4.f, layers);
            RaycastHit raycastHit2;
            Physics::Raycast(GameObject::Find("LeftHand Controller")->GetTransform()->GetPosition(),
                             GameObject::Find("LeftHand Controller")->GetTransform()->GetRight(),
                             raycastHit2, 4.f, layers);
            if (raycastHit2.distance > raycastHit.distance) {
                normal2 = raycastHit.normal;
                dist2 = raycastHit.distance;
            } else {
            ...
            if (dist2 < maxD2) {
                vel2 = normal2 * (9.8f * Time::GetDeltaTime());
                rig->SetVelocity(rig->GetVelocity() - vel2);
            } else {
                rig->SetUseGravity(true);
            }
